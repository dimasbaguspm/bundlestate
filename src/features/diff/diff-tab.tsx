...
export function DiffTab({ report }: { report: BundleStateReport }) {
  ...
  const diff = useMemo(
    () => (other ? diffReports(report, other) : null),
    [report, other],
  );
  ...
  if (others.length === 0) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-2 text-dim">
        ...
        <p className="text-xs text-faint">Analyze a second bundle (main vs feature) to see the delta.</p>
      </div>
    );
  }
      ...
      {diff && (
        <>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            <Stat label="Δ Raw" value={fmtSigned(diff.totalRawDelta)} tone={tone(diff.totalRawDelta)} />
            <Stat
              label="Δ Gzip"
...
function Stat({ label, value, tone = "neutral" }: { label: string; value: string; tone?: "ok" | "danger" | "neutral" }) {
  const color =
    tone === "ok" ? "text-[var(--tint-emerald-fg)]" : tone === "danger" ? "text-[var(--tint-rose-fg)]" : "text-ink";
  return (
