...
function report(assets: BundleStateReport["assets"]): BundleStateReport {
  return {
    id: "r",
    sourceName: "app.zip",
...
describe("buildPackageSizes", () => {
  ...
  it("skips assets with no known packages", () => {
    expect(buildPackageSizes(report([{ name: "x.js", sizeBytes: 10, gzipBytes: 5, usedModules: [] }]))).toEqual([]);
  });
});
