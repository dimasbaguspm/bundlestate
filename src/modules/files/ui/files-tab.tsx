...
const MonacoPreview = lazy(() =>
  import("@/components/monaco-preview").then((m) => ({ default: m.MonacoPreview })),
);
  ...
  const preview = useMemo(() => {
    ...
    if (stat) {
      if (stat.rawBytes) {
        return {
          title: stat.path,
          lang: langFor(stat.path),
          value: decodeText(stat.rawBytes),
      ...
      return {
        ...
        value: "",
        bytes: stat.sizeBytes,
        note: "Raw content was not extracted for this file (too large or skipped by the parser).",
      };
    }
    return null;
  }, [selected]);
