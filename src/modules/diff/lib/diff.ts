...
export function diffReports(base: BundleStateReport, other: BundleStateReport): ReportDiff {
  ...
  packages.sort(
    (a, b) =>
      Math.abs(b.deltaBytes) - Math.abs(a.deltaBytes) || a.fullName.localeCompare(b.fullName),
  );
  ...
  const resolvedCycles = base.insights.circularDepGroups.filter(
    (g) => !otherCycles.has(cycleKey(g)),
  );
