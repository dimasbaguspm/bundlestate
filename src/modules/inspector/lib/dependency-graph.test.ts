...
import { buildPackageGraph } from "./dependency-graph";
import type { BundleStateReport, ModuleGraph } from "@/utils/types";

const moduleGraph = (
  edges: ModuleGraph["edges"],
  pkgModules: ModuleGraph["pkgModules"],
): ModuleGraph => ({
  nodes: [
...
describe("buildPackageGraph", () => {
  it("aggregates module edges into weighted package edges with an app node", () => {
    ...
    const edge = (s: string, t: string) => data.edges.find((e) => e.source === s && e.target === t);
  ...
  it("falls back to lockfile pkgToSubPkg edges when no module graph exists", () => {
    ...
    expect(data.edges).toEqual([{ source: "react", target: "scheduler", weight: 1 }]);
  ...
  it("shows a package that ships multiple versions as separate nodes", () => {
    const moduleGraph: ModuleGraph = {
      nodes: [
        {
          id: "node_modules/.pnpm/foo@1.0.0/node_modules/foo/index.js",
          pkg: "foo",
          version: "1.0.0",
          local: false,
        },
        {
          id: "node_modules/.pnpm/foo@2.0.0/node_modules/foo/index.js",
          pkg: "foo",
          version: "2.0.0",
          local: false,
        },
