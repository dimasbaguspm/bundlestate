 ...
 */
export function findLineages(graph: ModuleGraph, targetIds: string[]): LineageResult {
  ...
  const roots = [...reachable].filter((id) => !incoming.has(id) || incoming.get(id)!.length === 0);
...
function pruneSamePackageRun(path: string[], byId: Map<string, { pkg?: string }>): string[] {
  const out: string[] = [];
